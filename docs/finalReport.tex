...
{\color{accent!70!white}\fontsize{13}{20}\selectfont
  CS 6650 --- Building Scalable Distributed Systems\par}
\vspace{0.15cm}
{\color{white!75!primary}\normalsize
  Northeastern University \textperiodcentered{}
  Final Project Report \textperiodcentered{}
  April 2026\par}
...
\begin{tcolorbox}[
  ...
  \vspace{6pt}
  \begin{tabular}{@{}ll@{}}
    \textbf{Anand Mohan Singh}    & Infrastructure (Terraform, 9 modules, Makefile) \\[3pt]
...
\begin{tcolorbox}[
  ...
  \vspace{4pt}
  \url{https://github.com/anandms101/CampusCloud-Print-Queue}
\end{tcolorbox}
...
% ============================================================
\section{Video Presentation [5 marks]}
\label{sec:video}

The submission includes one screen-capture video in which each team member presents the portion of the project they personally built. No AI voice-overs are used; all narration is performed live by the respective team member. The video is segmented as follows.

\subsection{Segment 1 --- Anand Mohan Singh (2--3 minutes)}
...
\begin{tcolorbox}[
  ...
  \textbf{Video link:} \url{https://github.com/anandms101/CampusCloud-Print-Queue} (linked from README)
\end{tcolorbox}


% ============================================================
\section{Code and Repository [5 marks]}
\label{sec:code}
...
% ============================================================
\section{Project Management [5 marks]}
\label{sec:pm}
...
\midrule
Terraform (9 modules)    & VPC, ECR, IAM, DynamoDB, S3, SQS, ALB, ECS, CloudWatch & Anand & \textcolor{success}{Done} \\
Makefile automation      & 20+ targets: deploy, teardown, logs, queue-depth, e2e test & Anand & \textcolor{success}{Done} \\
CI/CD (GitHub Actions)   & 4-job pipeline: go vet, pytest, terraform validate, docker build & Anand & \textcolor{success}{Done} \\
Go Gin API service       & 7 handlers, middleware, circuit breaker, bulkhead, rate limiter & Vaibhav & \textcolor{success}{Done} \\
DynamoDB schema          & Table, GSI (userId-createdAt-index), TTL, conditional expressions & Vaibhav & \textcolor{success}{Done} \\
FastAPI variant          & Python API alternative with Bulkhead isolation on POST /jobs & Vaibhav & \textcolor{success}{Done} \\
Unit tests (Go + Python) & 25 Go tests (mocks), 10 Python tests (moto) & Vaibhav + Pranav & \textcolor{success}{Done} \\
Printer worker           & SQS poll loop, idempotency state machine, heartbeat, S3 cleanup & Pranav & \textcolor{success}{Done} \\
Experiment scripts       & Locust (Exp 1), asyncio/aiohttp (Exp 2), requests (Exp 3), AWS CLI (Exp 4) & Pranav & \textcolor{success}{Done} \\
CloudWatch observability & Log groups (7-day retention), 6-panel dashboard, structured JSON logging & Pranav & \textcolor{success}{Done} \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{itemize}[noitemsep]
  ...
  \item \textbf{Experiment 3 rate limiting.} During saturation testing, rapid bulk job uploads triggered the API's 100 req/s rate limiter (429 responses). We adjusted the upload pacing to 0.7 second intervals between releases, which brought the effective rate below the limiter threshold for the release endpoint. The 27 failed releases in the final run were due to this rate limit being hit before the pacing adjustment was fully applied, and are documented as a known limitation of the experiment configuration.
\end{itemize}
...
\textbf{Configuration:}
\begin{itemize}[noitemsep]
  \item For each concurrency level $N \in \{2, 5, 10, 20, 50\}$: create one new \texttt{HELD} job, fire $N$ concurrent release requests targeting different printers simultaneously via \code{asyncio.gather}.
...
\begin{claimbox}
Per-printer SQS queues isolate throughput between printers. Under overload (more jobs released than a single worker can drain in real time), the queue absorbs the backlog without dropping or corrupting jobs. Latency degrades linearly with queue depth, not through errors.
\end{claimbox}
...
\subsubsection{Methodology}
\textbf{Tool:} Python \code{requests} library (\code{tests/experiment3\_saturation/saturation\_test.py})

\textbf{Configuration:}
\begin{itemize}[noitemsep]
  \item 50 jobs created and release-attempted to \code{printer-1} at 0.7 s intervals.
  ...
  \item Metrics collected: total drain time, per-job end-to-end latency (release to DONE), error count, SQS queue depth (peak), job disposition (completed / queued-pending / failed-to-release).
\end{itemize}
...
Jobs successfully released                & 27 \\
Failed to release (rate limited, 429)     & 27 \\
Jobs completed (DONE)                     & 23 / 23 released-and-processed \\
Timeouts or lost jobs                     & 0 \\
SQS queue depth (peak)                    & \textasciitilde23 messages \\
...
% Doughnut workaround: just a legend-style table
\end{axis}
\end{tikzpicture}
...
\begin{resultbox}
All 23 successfully-released jobs completed without error or loss, even though the queue peaked at 23 pending messages. The system behaved as designed: under overload, jobs queue in SQS and drain sequentially at the worker's throughput ceiling (one job per 5--15 s). No jobs were dropped, no DynamoDB states were corrupted, and no SQS messages were lost or sent to the DLQ.
...
The 27 failed releases (HTTP 429) are a consequence of the API's rate limiter interacting with the experiment's release pacing. These failures are a feature, not a bug: the rate limiter protected the API under burst traffic. A production system would use an exponential-backoff retry loop on the client side. The 4 jobs in the graphs described as ``released (pending)'' were in flight at the time of result collection and subsequently completed.
\end{resultbox}

\begin{limitbox}
The experiment tested a single printer in isolation. We did not run simultaneous load on printer-2 and printer-3 to measure cross-printer isolation empirically (it was confirmed by inspection: the other workers' SQS queues showed zero depth during the run). A multi-printer saturation test with concurrent job floods to all three printers would provide a stronger isolation proof. Additionally, we did not simulate worker auto-scaling (there is none --- fixed at 1 task per printer), so the saturation behavior is deterministic and representative of the physical model.
\end{limitbox}
...
\subsubsection{Methodology}
\textbf{Tool:} Python \code{requests} + AWS CLI (\code{tests/experiment4\_fault\_injection/fault\_injection\_test.py})

\textbf{Configuration:}
\begin{itemize}[noitemsep]
  \item Release 8 jobs to \code{printer-3}.
  \item Wait 10 seconds for the worker to begin processing.
  \item Force-stop the printer-3 ECS task via \code{aws ecs stop-task}.
  \item Poll DynamoDB every 5 seconds until all 8 jobs reach \texttt{DONE}.
  \item Verify: zero duplicate \texttt{DONE} writes, zero stuck jobs, zero \texttt{FAILED} jobs.
\end{itemize}
...
% ============================================================
\section{Community Contributions [5 marks]}
\label{sec:community}

\subsection{Piazza Posting}
Under the ``Final Projects'' section on Piazza, we will post:
\begin{enumerate}[noitemsep]
  \item A link to our screen-capture video (one segment per team member).
  \item A link to this experiments report (PDF).
  \item A link to the public GitHub repository.
\end{enumerate}

\subsection{Related Projects}

We will identify and comment on three Piazza final project postings most similar to ours. Our expected comparison dimensions are below.

\subsubsection{Expected Similarity Criteria}
Projects most similar to ours will share at least two of the following traits:
\begin{itemize}[noitemsep]
  \item Message-queue-driven asynchronous processing (SQS, RabbitMQ, Kafka).
  \item Optimistic concurrency or conditional-write patterns for distributed state.
  \item ECS or Kubernetes-based stateless API with external state in a managed database.
  \item At-least-once delivery with application-level idempotency.
\end{itemize}

\subsubsection{Comparison Template (to be completed post-posting)}
For each of the three selected projects, our Piazza comment will address:

\textbf{Similarities:} Which architectural primitives do we share? (e.g., both use SQS for work distribution; both use DynamoDB conditional writes for concurrency control.)

\textbf{Differences:} What design choices diverged and why? (e.g., their system uses FIFO SQS for ordering guarantees, which we traded for higher throughput with application idempotency; their API is stateful with in-memory caches, which we avoided for simpler horizontal scaling.)

\textbf{What we learned:} What would we adopt from their design if we were to rebuild? (e.g., their use of DynamoDB Streams for audit logging is an approach we would add to our observability layer; their circuit breaker configuration with shorter probe intervals reduces recovery latency compared to our 15-second open window.)
...
\begin{tcolorbox}[
  ...
  \textbf{Note:} Specific project links and completed comparison text will be added to this section after the Piazza posting deadline, when peer projects are available for review.
\end{tcolorbox}
